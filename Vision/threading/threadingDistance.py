...
rightStream = urllib.request.urlopen('http://192.168.0.90/mjpg/video.mjpg')
 
leftBytes = b''
rightBytes = b''

leftSide = 'Left camera = '
rightSide = 'Right camera = '
...
def capture(stream, bytes, dist, side):
...
#capture(leftStream, leftBytes, left)

a = threading.Thread(target=capture, args=[leftStream, leftBytes, distLeft, leftSide])
b = threading.Thread(target=capture, args=[rightStream, rightBytes, distRight, rightSide])
